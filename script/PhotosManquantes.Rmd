...
test6<-Dataphoto
test<- test6 %>% filter(Species=="Argyrosomus regius" | Species=="Chelidonichthys cuculus" | Species=="Clupea clupea" | Species=="Engraulis encrasicolus" | Species=="Dicentrarchus labrax" | Species=="Flat fish sp." | Species=="Gadidae sp." | Species=="Merluccius merluccius" | Species=="Mullus sp." | Species=="Lophius sp." | Species=="Osmerus eperlanus" | Species=="Pagellus bogaraveo" | Species=="Sardina pilchardus" | Species=="Scomber scombrus" | Species=="Scophthalmus sp." | Species=="Solea solea" | Species=="Sprattus sprattus")
colnames(test)[8]<-"ID"
...
test9<-test9%>%replace(.=="NULL","COMPLETE")
...
```{r FRMED, echo=F, warning=F, message=F,fig.height=10,fig.width=6.8,out.width='475px',fig.align='center'}
test11<-data.frame(Photo=rep(imgspp,length(1)),Espèce=paste(test10$sppfr,test10$spplat,sep=" / "),Femelle=test10$F,Male=test10$M) %>% 
  separate(`Espèce`, c("french_name", "latin_name"), sep = "\\ / ") %>% 
  mutate(latin_name = paste0(" / ", latin_name) )
